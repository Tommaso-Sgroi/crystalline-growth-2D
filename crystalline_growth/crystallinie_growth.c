...
void move_and_precrystalize(arraylist* particles, arraylist* precrystalize, struct space* space, size_t iterazioni){

    for(size_t k = 0; k < iterazioni; k++){
        if(particles->used > 0){
            for(size_t i = particles->used - 1 ;; i--){
                ...
                if(check_crystal_neighbor(space,  p)){
                    ...
                    do{
                        x_movement =  p->x + (rand()%2 * (rand()%2? 1: -1)); // pick random x direction
                        y_movement =  p->y + (rand()%2 * (rand()%2? 1: -1)); // pick random y direction
            ...
            if(precrystalize->used > 0){
                trim_list(precrystalize); //pericoloso da usare, non l'avevo testato e non ricordo se funzionasse bene
            }
...
void build_vector_particle(arraylist* particles, int numero_particelle, size_t len_x, size_t len_y, int posizione_seed_x, int posizione_seed_y){
    for (int i=0; i<numero_particelle; i++){
        ...
        do
        {
            info->x = RAND_SIZE_T() % len_x;
            info->y = RAND_SIZE_T() % len_y;
